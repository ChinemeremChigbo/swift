...
class ObjectUpdater(Daemon):
    ...
    def run_forever(self, *args, **kwargs):
        ...
        while True:
            ...
            self.begin = time.time()
            self._process_devices(is_forever=True)
    ...
    def run_once(self, *args, **kwargs):
        ...
        self._process_devices(is_forever=False)
        elapsed = time.time() - self.begin
        self.logger.info(
    ...
    def _process_devices(self, is_forever):
        """Process devices, handling both single and multi-threaded modes."""
        pids = []
        # read from container ring to ensure it's fresh
        self.get_container_ring().get_nodes('')
        self._all_local_devices = self._listdir(self.devices)
        for device in self._all_local_devices:
            try:
            ...
            if is_forever:
                ...
                if pid:
                    ...
                    self._process_device_in_fork(dev_path, device)
                    sys.exit()
            else:
                # Single-threaded handling
                self.object_sweep(dev_path)

        # Wait for all child processes to finish in multi-threaded mode
        if is_forever:
            while pids:
                pids.remove(os.wait()[0])

    def _process_device_in_fork(self, dev_path, device):
    ...
    def aggregate_and_dump_recon(self, elapsed):
        ...
        devices_to_remove = set(device_stats) - set(self._all_local_devices)
